...
def evaluation_1st(initial_insruction_path, eval_path, ollama_model, save_path):
    with open(initial_insruction_path, 'r', encoding='utf-8') as f:
        initial_insructions = json.load(f)
        
    with open(eval_path, 'r', encoding='utf-8') as f:
        eval_data = json.load(f)
    eval_questions = eval_data["eval_1"]["Question"]
    eval_essay = eval_data["eval_1"]["Essay"]
    eval_score = eval_data["eval_1"]["Overall"]
    
    prompt_sets = evaluate_prompt(initial_insructions, eval_questions, eval_essay)
    ...
    all_results = {}  
    for idx in initial_insructions:
        all_results[idx] = {
            "instruction": initial_insructions[idx],
...
def evaluate_prompt(initial_insructions, eval_questions, eval_essay):
    prompt_sets = {}

    for instruction_idx in initial_insructions:
        input_prompt = []
        
        for question, answer in zip(eval_questions, eval_essay):
            prompt = f"""{initial_insructions[instruction_idx]}
...
if __name__ == "__main__":
    class CONFIG:
        initial_insruction_path="./results/ielts/initial_instruction/instruction_pool.json"
        eval_path="./data/input/ielts/eval_sets.json"
        ollama_model = "llama3:70b-instruct"
        save_path="./results/ielts/initial_instruction/results.json"
        
    evaluation_1st(CONFIG.initial_insruction_path, CONFIG.eval_path, CONFIG.ollama_model, CONFIG.save_path)